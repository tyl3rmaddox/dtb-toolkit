...
// lblakey: Tried with more aggressive error reporting
error_reporting(-1); 
...
function runHindPrep() {
	$id = $_GET["id"];
	$url = $_GET["url"];
	$cmd = sprintf("/opt/nlsval/run-hindprep.sh %s %s", $id, $url);
	disable_ob();
	echo exec('whoami');
	echo "<pre>";
        execute ($cmd);
...
function runNlsValidator () {
	...
	$cmd = sprintf("/opt/nlsval/run-validator-hind.sh %s %s", $id, $url);
	disable_ob();
	echo exec('whoami');
	echo "<pre>";
	execute ($cmd);
